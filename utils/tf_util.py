...
def get_edge_feature(point_cloud, nn_idx, k=20):
  ...
  dim_size = len(point_cloud.get_shape().as_list())
  if dim_size == 4:
    point_cloud = tf.squeeze(point_cloud, axis=2)  # (batch_size, num_points,num_dims)
  ...
  point_cloud_neighbors = tf_sampling.gather_point(point_cloud, nn_idx) #(B,N,k,3)
  ...
  # h(x,x-x_i)
  return edge_feature # BxN*K*(2*3)

def get_edge_feature_NS(point_cloud, nn_idx, k=20):
  """Construct edge feature for each point
  Args:
    point_cloud: (batch_size, num_points, 1, num_dims)
    nn_idx: (batch_size, num_points, k)
  ...
  point_cloud_neighbors = tf_sampling.batch_gather_point(point_cloud, nn_idx) #(B,N,k,3)
