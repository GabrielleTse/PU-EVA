...
def coordinate_reconstruction_unit(inputs,scope="reconstruction",is_training=True,bn_decay=None,use_bias=False):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        
        net = conv2d(inputs, 512, [1, 1],
                    padding='VALID', stride=[1, 1],
                    bn=False, is_training=is_training,
                    scope='conv0', bn_decay=bn_decay,use_bias=use_bias)
        net = conv2d(net, 512, [1, 1],
                    padding='VALID', stride=[1, 1],
                    bn=False, is_training=is_training,
                    scope='conv1', bn_decay=bn_decay,use_bias=use_bias)

        coord = conv2d(net, 128, [1, 1],
                    padding='VALID', stride=[1, 1],
                    bn=False, is_training=is_training,
                    scope='conv2', bn_decay=bn_decay,use_bias=use_bias)

        coord = conv2d(coord, 3, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=False, is_training=is_training,
                            scope='fc_2', bn_decay=None,
                            activation_fn=None, use_bias=use_bias)

    return coord


def attention_point_extend_unit(inputs,xyz,up_ratio,k,FLAGS=None,scope='attention_unit', is_training=True):
    """
    inputs: (B,N,1,480)
    """
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        neighbors_features_1,_, _ = get_neigh_feature(inputs,xyz, k=up_ratio,k_max=k, random=True,args=FLAGS)  # (B,N,r,C)
        neighbors_features_2,xyz_neighbors_2, _ = get_neigh_feature(inputs,xyz, k=k,k_max=k, random=False,args=FLAGS)  # (B,N,k,C)

        ############
        # baseline_new
        ##################
        g = conv2d(neighbors_features_1, 512, [1, 1],
                   padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training,
                   scope='conv_f', bn_decay=None)

        h = conv2d(neighbors_features_2,512, [1, 1],
                   padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training,
        ...
        beta = tf.nn.softmax(s/(k+0.0),dim=-1)
        #####################
        local_f = conv2d(neighbors_features_2, 512, [1, 1],
                   padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training,
                   scope='local_f', bn_decay=None)

        coarse = tf.matmul(beta,xyz_neighbors_2) # (B,N,r,k ) (B,N,K,3) ==> (B,N,r,3) 
        o = tf.matmul(beta,local_f)
...
def get_neigh_feature(point_cloud,xyz, k=16,k_max=16, random=False,args=None):
    ...
    point_num = tf.shape(xyz)[1]
    _, idx = knn_point_2(k_max+1, xyz, xyz, unique=True, sort=True) 
    
    if random:
        x = np.random.rand(args.batch_size,256,int(k_max))
        index = x.argsort(-1)[:,:,:k] 
        point_indices  = tf.convert_to_tensor(index.astype(np.int32))
        batch_indices = tf.tile(tf.reshape(tf.range(batch_size), (-1, 1, 1, 1)), (1, point_num, k, 1))
        N_indices =  tf.tile(tf.reshape(tf.range(point_num), (1, -1, 1, 1)), (batch_size, 1, k, 1))
        indices = tf.concat([batch_indices,N_indices, tf.expand_dims(point_indices, axis=3)], axis=3) 
        idx = tf.gather_nd(idx, indices)
    else:
        idx = idx[:, :, 1:(k+1), :]
        
    point_cloud_neighbors = tf.gather_nd(point_cloud, idx)
    point_cloud_central = tf.expand_dims(point_cloud, axis=-2)
    point_cloud_central = tf.tile(point_cloud_central, [1, 1, k, 1]) #(B,N,K,3)
    
    xyz_neibors = tf.gather_nd(xyz, idx)
    xyz_central = tf.expand_dims(xyz, axis=-2)
    xyz_central = tf.tile(xyz_central, [1, 1, k, 1]) #(B,N,K,3)
    d_xyz = tf.expand_dims(tf.reduce_sum(tf.pow(xyz_central -xyz_neibors,2 ),axis=-1),axis=-1)
    d_features = tf.expand_dims(tf.reduce_sum(tf.pow(point_cloud_neighbors - point_cloud_central,2 ),axis=-1),axis=-1)

    neigh_feature = tf.concat(
        [
        point_cloud_central,
        point_cloud_neighbors, 
        point_cloud_neighbors - point_cloud_central,
        d_features,
        xyz_neibors-xyz_central
        ], axis=-1)

    return neigh_feature,tf.gather_nd(xyz, idx), idx


def get_edge_feature(point_cloud,xyz, k=16, idx=False):
    """Construct edge feature for each point
    Args:
        point_cloud: (batch_size, num_points, 1, num_dims)
        nn_idx: (batch_size, num_points, k, 2)
    ...
    """
    if idx:
        _, idx = knn_point_2(k, xyz, xyz, unique=True, sort=True)
    else:
...
def dense_conv(feature, n=3,growth_rate=64, k=16, scope='dense_conv',**kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y, idx = get_edge_feature(feature,feature, k=k, idx=None)  # [B N K 2*C]
        for i in range(n):
            if i == 0:
                y = tf.concat([
