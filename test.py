...
def eval(model_path,gt_dir,pred_dir,out_num):
    # print(GT_DIR)
    # GT_DIR = gt_dir
    ...
    with tf.Session() as sess:
        ...
        print("{:6s} ".format("name"), "|".join(["{:>10s}".format(d) for d in fieldnames[1:]]))
        for D in [PRED_DIR]:
            ...
            with open(os.path.join(model_path+"-{}-evaluation.csv".format(out_num)), "w") as f:
                ...
                for gt_path, pred_path in gt_pred_pairs:
                    ...
                    print(cd_backward_value.shape,cd_forward_value.shape)
                    print(np.amax(cd_forward_value, axis=0))
